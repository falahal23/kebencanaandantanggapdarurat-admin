...
<div class="container mx-auto px-4 py-6">
    <div class="bg-white shadow rounded-lg p-6">
        ...
        <form action="{{ route('admin.posko.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <input type="text" name="nama" value="{{ old('nama') }}" class="w-full border rounded p-2 mb-4" required>

            <!-- Alamat -->
            <label class="block mb-2 font-medium">Alamat</label>
            <input type="text" name="alamat" value="{{ old('alamat') }}" class="w-full border rounded p-2 mb-4" required>
            ...
            <input type="text" name="penanggung_jawab" value="{{ old('penanggung_jawab') }}" class="w-full border rounded p-2 mb-4">
            ...
            <select name="kejadian_id" class="w-full border rounded p-2 mb-4" required>
                ...
                @foreach ($kejadian as $item)
                    <option value="{{ $item->kejadian_id }}" {{ old('kejadian_id') == $item->kejadian_id ? 'selected' : '' }}>
                        {{ $item->jenis_bencana }} | {{ $item->lokasi_text }} | {{ \Carbon\Carbon::parse($item->tanggal)->format('d M Y') }}
                    </option>
                @endforeach
            </select>
            ...
            <div class="flex justify-end space-x-2">
                <button type="submit" class="px-6 py-3 font-bold text-white bg-pink-600 rounded-lg hover:bg-pink-700 transition">
                    <i class="fa fa-save mr-1"></i> Simpan Data
                </button>

                <a href="{{ route('admin.posko.index') }}" class="px-6 py-3 font-bold text-white bg-gray-600 rounded-lg hover:bg-gray-700 transition">
                    ← Kembali
                </a>
            </div>
        </form>
