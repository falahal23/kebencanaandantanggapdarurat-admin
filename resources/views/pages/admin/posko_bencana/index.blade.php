        ...
        <div class="w-full max-w-full px-3 mt-0 lg:w-12/12 lg:flex-none">
            <div
                ...
                <div class="border-black/12.5 mb-0 rounded-t-2xl border-b-0 border-solid bg-white p-6 pb-0">
                    ...
                    <!-- SEARCH & FILTER -->
                    <div>
                        <form action="{{ route('posko.index') }}" method="GET" class="mb-6" <!-- Label -->
                            <label class="block font-semibold mb-1">Pencarian</label>
                            <form method="GET"
                                class="mb-6 bg-white p-4 rounded-lg shadow flex flex-wrap items-center gap-3">
                                ...
                                <select name="alamat" class="p-2 border rounded-lg">
                                    ...
                                    @foreach ($alamatList as $alamat)
                                        <option value="{{ $alamat }}"
                                            {{ request('alamat') == $alamat ? 'selected' : '' }}>
                                            {{ $alamat }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                @if (request()->has('search') && request('search') != '')
                                    <a href="{{ route('posko_bencana.index') }}"
                                        class="px-4 py-2 h-10 bg-gray-300 text-black rounded-lg shadow hover:bg-gray-400">
                                        ⟲
                                    </a>
                                @endif
                            ...
                            <table class="items-center w-full mb-0 align-top border-gray-200 text-slate-500">
                                <thead class="align-bottom bg-gray-100">
                                    <tr>
                                        ...
                                        <th class="px-6 py-3 font-bold text-left uppercase text-xxs text-slate-600">Nama
                                            Posko
                                        </th>
                                        <th class="px-6 py-3 font-bold text-left uppercase text-xxs text-slate-600">Alamat
                                        </th>
                                        <th class="px-6 py-3 font-bold text-left uppercase text-xxs text-slate-600">Kontak
                                        </th>
                                        <th class="px-6 py-3 font-bold text-left uppercase text-xxs text-slate-600">
                                            Penanggung Jawab</th>
                                        <th class="px-10 py-3 font-bold text-center uppercase text-xxs text-slate-600">Aksi
                                        </th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @forelse($posko as $index => $item)
                                        <tr>
                                            ...
                                            <td class="p-2 align-middle bg-transparent border-b whitespace-nowrap">
                                                <span
                                                    class="text-xs font-semibold">{{ $item->penanggung_jawab ?? '-' }}</span>
                                            </td>
